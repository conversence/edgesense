...
def extract_dpsg(mdg, ts, team=True):
    ...
    for node in mdg:
        nts = mdg.nodes[node].get('created_ts', None)
        if nts and nts <= ts and (team or not mdg.nodes[node]['team']):
            dg.add_node(node, **mdg.nodes[node])
...
def build_network(network):
    ...
    for edge in network['edges']:
        MDG.add_edge(edge['source'], edge['target'], **edge)
